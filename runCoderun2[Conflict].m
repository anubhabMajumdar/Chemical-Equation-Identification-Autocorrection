...
im = imread('C:\Users\Anubhab Majumdar\Google Drive\SKM\FINAL_RESULT\cbook (9)\cbook (9).jpg');
bin=im2bw(im);
BW=~bin;
...
%% to check if each and every equation is extracted successfully
 for i=1:eqn
    imshow(BW(eqline_t(i):eqline_b(i),:)),title(['Line no: ',num2str(i)]);
 end
...
for i=1:eqn
    ...
    imshow(equation),title('original equation');
    ...
    imshow(equation),title('eq');
    imshow(operatorLine),title('operator');
    imshow(operandLine),title('oprnd');
    ...
    imshow(finEqu),title('End Removal');
